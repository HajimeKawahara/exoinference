...
Let $A_j$ be the set of $\nu$ values whose range lies within a small region $\Delta \sigma$ around $\sigma = \sigma_j$.  In other word, defining the whole wavenumber set by $U = [\nu_\mathrm
{min}, \nu_\mathrm{max}]$, we define
\begin{align}
A_j = \{ \nu \in U | \sigma(\nu) \in [\sigma_j, \sigma_j + \Delta \sigma] \}.
\end{align}
Note that $A_i \cap A_j = \varnothing$ for $i \ne j$. The support of the wavenumber set that has the value can then be expressed as
\begin{align}
    V = \bigcup_{j=0}^{n} A_j.
\end{align}
...
Figure~\ref{fig:ckd_fig1} (right) shows $\sigma(g)$ obtained in this way—essentially the inverse of the cumulative distribution function of $g$. In other words, $\sigma(g)$ is the quantile function.  
