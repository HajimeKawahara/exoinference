...
最適化において、$\nv$の微分をゼロとする停留条件を用いるためには、$\nv \ge 0$の不等式制約条件について、 $\nv$の要素すべてが0でない(inactive)である必要がある。activeな要素($n_i=0$)の場合、最適値においてその微分値がゼロとは限らない。しかし、式(\ref{eq:Lpartialn})より、$n_i \to +0$で勾配が$\to -\infty$となるため、$n_i = 0$となることはない。これは後に$\nv$の対数を変数ととることの正当化ともなっている。\footnote{対照的に、凝縮相を含む系の場合、存在量が0になりうる（多くの凝縮種は0になる）ため、停留条件を用いることができるのは、非ゼロの種を同定でき、その凝縮種のみを扱う場合である。
}
